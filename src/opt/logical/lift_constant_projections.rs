...
use crate::node::{InstantVectorEval, StepVectorEval, StreamingRangeFunctionEval};
...
/// ```
///
/// **Pattern 6 – StreamingRangeFunctionEval over a projection with constants:**
///
/// ```text
/// StreamingRangeFunctionEval(label_columns)     Projection [constants + col refs]
///   Projection [cols + consts]              →     StreamingRangeFunctionEval(label_columns')
///     child                                         Projection [cols only]
///                                                     child
/// ```
///
        ...
        match &plan {
            ...
            LogicalPlan::Extension(ext)
                if ext
                    ...
                    .is_some() =>
            {
                rewrite_streaming_range_func_eval(plan)
            }
...
/// Pattern 6: Lift constant columns from a Projection child of a
/// StreamingRangeFunctionEval node. Non-timestamp, non-value constant columns
/// are removed from the inner projection and from `label_columns`, then added
/// back via an outer projection above the StreamingRangeFunctionEval.
fn rewrite_streaming_range_func_eval(plan: LogicalPlan) -> Result<Transformed<LogicalPlan>> {
    let LogicalPlan::Extension(ref ext) = plan else {
        unreachable!();
    };
    ...
    // Rebuild: Projection(constants + refs) -> StreamingRangeFunctionEval(updated) -> Projection(non-constants) -> child
    let LogicalPlan::Extension(ext) = plan else {
        unreachable!();
    };
    let eval = ext
    ...
    // Build new StreamingRangeFunctionEval with updated label_columns.
    let new_label_columns: Vec<String> = eval
        .label_columns
        .iter()
        .filter(|name| !constant_names.contains(name.as_str()))
