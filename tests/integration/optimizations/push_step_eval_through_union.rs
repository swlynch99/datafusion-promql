...
use datafusion_promql::opt::logical::PushStepEvalThroughUnion;
...
fn wrap_step_eval(input: LogicalPlan, label_columns: Vec<String>) -> LogicalPlan {
...
fn apply_rule(plan: LogicalPlan) -> (LogicalPlan, bool) {
    let rule = PushStepEvalThroughUnion;
    let Transformed {
        data, transformed, ..
...
/// When union branches have disjoint label constants, StepVectorEval should
...
fn test_push_down_disjoint_branches() {
    ...
    let plan = wrap_step_eval(union, vec!["__name__".to_string(), "instance".to_string()]);
    ...
    // Each branch should be a StepVectorEval.
    ...
    for (i, input) in union.inputs.iter().enumerate() {
        ...
        assert!(
            ext.node.as_any().downcast_ref::<StepVectorEval>().is_some(),
            "branch {i} should be StepVectorEval"
        );
    }
}

/// Three branches with unique label constants should all get StepVectorEval.
#[test]
fn test_push_down_three_branches() {
    ...
    let plan = wrap_step_eval(union, vec!["job".to_string()]);
    let (result, transformed) = apply_rule(plan);
    assert!(transformed);
...
fn test_no_push_down_identical_labels() {
    ...
    let plan = wrap_step_eval(union, vec!["__name__".to_string()]);
...
/// the rule should fire because the fingerprint tuple is unique.
#[test]
...
/// When the input to StepVectorEval is not a Union, the rule should not fire.
#[test]
fn test_no_push_down_non_union_input() {
    ...
    let plan = wrap_step_eval(proj, vec!["__name__".to_string()]);
    let (_, transformed) = apply_rule(plan);
    assert!(!transformed);
}
...
fn test_no_push_down_non_projection_branches() {
    ...
    let plan = wrap_step_eval(union, vec![]);
    let (_, transformed) = apply_rule(plan);
    assert!(!transformed);
}
...
/// The pushed-down StepVectorEval should preserve all parameters.
#[test]
fn test_push_down_preserves_parameters() {
    ...
    let eval = StepVectorEval::new(
        union,
        10_000_000_000, // start_ns
        70_000_000_000, // end_ns
