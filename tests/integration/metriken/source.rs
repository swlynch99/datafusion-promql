...
use arrow::array::{AsArray, Int64Array, ListArray, StructArray, UInt64Array};
...
use super::fixture::{FakeSnapshot, HistogramType, write_fixture};
...
/// per row.
fn build_three_snapshot_fixture(histogram_type: HistogramType) -> tempfile::NamedTempFile {
...
async fn standard_histogram_schema_and_deltas() {
    let fixture = build_three_snapshot_fixture(HistogramType::Standard);
...
async fn sparse_histogram_schema_and_deltas() {
    let fixture = build_three_snapshot_fixture(HistogramType::Sparse);
    let source = MetrikenMetricSource::try_new(fixture.path()).unwrap();
...
async fn scalar_columns_are_passed_through() {
    ...
    let fixture = write_fixture(snapshots, HistogramType::Standard);
    let source = MetrikenMetricSource::try_new(fixture.path()).unwrap();
