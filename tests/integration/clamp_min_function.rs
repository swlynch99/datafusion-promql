...
#[tokio::test]
async fn test_clamp_min_negative_threshold() {
    let source = make_source(vec![-10.0, 0.0]);
    let engine = PromqlEngine::new(Arc::new(source));
    ...
    match result {
        QueryResult::Vector(mut samples) => {
            ...
            // host1: clamp_min(-10.0, -5) = -5
            assert!(
                (samples[0].value - (-5.0)).abs() < f64::EPSILON,
                "expected -5.0, got {}",
                samples[0].value
            );
            // host2: clamp_min(0.0, -5) = 0
            assert!(
                (samples[1].value - 0.0).abs() < f64::EPSILON,
                "expected 0.0, got {}",
                samples[1].value
            );
        }
...
#[tokio::test]
async fn test_clamp_min_preserves_labels() {
    let source = make_source(vec![1.0]);
    let engine = PromqlEngine::new(Arc::new(source));

    let ts = chrono::Utc.timestamp_millis_opt(1000).unwrap();
    let result = engine
        .instant_query("clamp_min(cpu_usage, 0)", ts)
        .await
        .unwrap();
